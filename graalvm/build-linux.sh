...
bash -c "gu install native-image && native-image --enable-http --enable-https --no-server --no-fallback \
-H:+AllowIncompleteClasspath -jar /opt/$ARTIFACT.jar && mv $ARTIFACT /opt/output/$ARTIFACT-linux"
